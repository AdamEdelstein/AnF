...
enum EnemyState 	{ moveLeft = 0, moveRight = 1, moveUp = 2, moveDown = 3, idleLeft = 5, idleRight = 4, jumpAir = 6, enemyDie = 7, goHome = 8 }
...
private var controller 		 : Rigidbody2D; 	// get controller 
...
function Update () 
{
	...
	// move the controller
	//rigidbody2D.Move (velocity * Time.deltaTime); 	// move the controller
}
...
function PatrolLeft ()
{
	rigidbody2D.velocity.x = -moveSpeed * Time.deltaTime;		// move the controller to the left	
	animator.SetInteger("Direction", 1);
	isRight = false;
}
...
function PatrolUp ()
{
	rigidbody2D.velocity.y = moveSpeed * Time.deltaTime;		// move the controller to the left	
	animator.SetInteger("Direction", 2);
}
